...
export class AppModule {
  constructor(private configService: ConfigService) {
    ...
    console.log(`
      ...
      ║  Database: ${'[configured — URL hidden from logs]'.padEnd(42)} ║
      ║  Redis: ${(() => { try { return new URL(this.configService.get('REDIS_URL') || '').hostname; } catch { return 'configured'; } })().padEnd(45)} ║
      ╚═══════════════════════════════════════════════════════╝
    `);
  }
}
